...
const ImageGenView: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setErrorState('none');
    try {
      const url = await generateImage(prompt, size, useNanoPro);
      setImgUrl(url);
    } catch (err) {
            ...
            <div className="space-y-2">
              ...
              <div className="flex bg-black/40 p-1 rounded-xl border border-white/10">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${!useNanoPro && size === '1K' ? 'bg-indigo-500 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                  Nano Banana (Flash)
                </button>
                <button
                  onClick={() => setUseNanoPro(true)}
                  className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${useNanoPro ? 'bg-indigo-500 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                >
                  Nano Pro (High Quality)
                </button>
              </div>
            </div>
            ...
            {useNanoPro && (
              <div className="space-y-2 animate-in fade-in slide-in-from-left-2">
                ...
                <select 
                  ...
                  className="p-2 bg-gray-900 border border-white/10 rounded-xl outline-none text-white focus:border-indigo-500/50 shadow-inner text-xs font-bold h-[42px]"
                >
